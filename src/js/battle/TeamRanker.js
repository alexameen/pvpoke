    ...
    function rankerObject() {
      ...
      this.rank = function(team, league, cup, exclusionList, context) {
        ...
        for (var i = 0; i < team.length; i++) {
          ...
          if (
            overrideSettings[0].ivs != "gamemaster" &&
            overrideSettings[0].ivs != "original"
          ) {
        ...
        if (
          (targets.length == 0 || cup.name != "custom") &&
          context != "matrix"
        ) {
        ...
        for (var i = 0; i < rankCount; i++) {
          ...
          if (
            overrideSettings[1].ivs != "gamemaster" &&
            overrideSettings[1].ivs != "original"
          ) {
          ...
          for (var n = 0; n < team.length; n++) {
            ...
            if (shieldMode == "single") {
              shieldTestArr.push([
                overrideSettings[0].shields,
                overrideSettings[1].shields
              ]);
            ...
            for (var j = 0; j < shieldTestArr.length; j++) {
              ...
              var opRating = Math.floor(
                (opHealthRating + opDamageRating) * 500
              );
            ...
            if (shieldTestArr.length > 1) {
              avgPokeRating = Math.round(
                Math.pow(
                  shieldRatings[0] * Math.pow(shieldRatings[1], 3),
                  1 / 4
                )
              );
            }
            ...
            csv +=
              "," + avgOpRating + "," + opponent.energy + "," + opponent.hp;
            ...
            if (pokemon.overall && pokemon.scores[4]) {
              score *=
                Math.sqrt(pokemon.overall) * Math.pow(pokemon.scores[4], 1 / 4);
            } else {
