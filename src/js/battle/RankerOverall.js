      ...
      this.displayRankingData = function() {
            ...
            if (!rankings[n]) {
              ...
              if (moveUsageMode == "aggregate") {
                ...
                for (var j = 0; j < rankObj.moves.fastMoves.length; j++) {
                  rankings[n].moves.fastMoves[j].uses +=
                    rankObj.moves.fastMoves[j].uses;
                }

                for (var j = 0; j < rankObj.moves.chargedMoves.length; j++) {
                  rankings[n].moves.chargedMoves[j].uses +=
                    rankObj.moves.chargedMoves[j].uses;
                }
              }
            }
          }
        ...
        for (var i = 0; i < rankings.length; i++) {
          ...
          if (arr.length > 2 && arr[2] != "0") {
            ...
            var effectivenessScenarios = [[1, 1]];
            ...
            for (var n = 0; n < effectivenessScenarios.length; n++) {
              ...
              chargedMoves[0].dpe =
                (chargedMoves[0].damage / chargedMoves[0].energy) *
                effectivenessScenarios[n][0];
              chargedMoves[1].dpe =
                (chargedMoves[1].damage / chargedMoves[1].energy) *
                effectivenessScenarios[n][1];
              chargedMoves.sort((a, b) =>
                a.dpe > b.dpe ? -1 : b.dpe > a.dpe ? 1 : 0
              );
              ...
              var cycleFastMoves = Math.ceil(
                chargedMoves[0].energy / fastMove.energyGain
              );
              ...
              if (
                ...
                (chargedMoves[0].energy == chargedMoves[1].energy &&
                  chargedMoves[1].moveId == "ACID_SPRAY")
              ) {
                factor =
                  cycleFastDamage / cycleDamage +
                  (chargedMoves[0].damage / cycleDamage) *
                    (chargedMoves[1].dpe / chargedMoves[0].dpe);
              }
            ...
            consistencyScore = Math.pow(
              consistencyScore,
              1 / effectivenessScenarios.length
            );
          }
          ...
          rankings[i].score = Math.pow(
            Math.pow(bestScore, 12) *
              Math.pow(sortedScores[0], 6) *
              Math.pow(sortedScores[1], 1) *
              Math.pow(sortedScores[2], 1) *
