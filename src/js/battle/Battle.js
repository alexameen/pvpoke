...
function Battle() {
      ...
      for (var i = poke.statBuffs.length - 1; i >= 0; i--) {
        if (poke.statBuffs[i] != 0) {
          turnMessages.push({
            index: poke.index,
            str:
              statNames[i] + " " + statDescriptions[poke.statBuffs[i] + 4] + "!"
          });
        }
      }
    }
  ...
  this.calculateBreakpoint = function(damage, defense, effectiveness, move) {
    ...
    var attack =
      ((damage - 1) * defense) /
      (move.power * move.stab * effectiveness * 0.5 * bonusMultiplier);
  ...
  this.step = function() {
    ...
    if (mode == "emulate") {
      pokemon[0].priority = Math.random() > 0.5 ? 1 : 0;
    ...
    if (turns > lastProcessedTurn) {
      for (var i = 0; i < 2; i++) {
        ...
        if (action) {
          ...
          if (
            action.type == "switch" ||
            (action.type != "switch" && poke.hp > 0 && opponent.hp > 0)
          ) {
            if (action.type == "fast" && mode == "emulate") {
              ...
              self.pushAnimation(
                poke.index,
                "fast",
                pokemon[action.actor].fastMove.cooldown / 500
              );
            }
    ...
    turnActions.sort((a, b) =>
      a.settings.priority > b.settings.priority
        ? -1
    ...
    for (var n = 0; n < turnActions.length; n++) {
      ...
      switch (action.type) {
          ...
          for (var j = 0; j < turnActions.length; j++) {
            if (turnActions[j].actor != action.actor) {
              if (turnActions[j].type == "fast") {
                ...
                if (
                  (opponent.cooldown == 0 &&
                    poke.hp <= pokemon[turnActions[j].actor].fastMove.damage) ||
                  poke.hp < 1
                ) {
                  lethalFastMove = true;
                }
        ...
        case "switch":
          if (
            (poke.cooldown == 0 && players[poke.index].getSwitchTimer() == 0) ||
            poke.hp < 1
          ) {
            action.valid = true;
          }
    ...
    for (var i = 0; i < 2; i++) {
      ...
      if (poke.hp <= 0) {
        timeline.push(
          new TimelineEvent("faint", "Faint", poke.index, time, turns)
        );
    ...
    if (mode == "emulate" && faintedPokemonIndexes.length > 0) {
      ...
      if (
        players[0].getRemainingPokemon() > 0 &&
        players[1].getRemainingPokemon() > 0
      ) {
        ...
        if (phaseProps.actors.indexOf(1) > -1) {
          ...
          if (
            players[1].getAI().hasStrategy("WAIT_CLOCK") &&
            players[1].getSwitchTimer() > 0 &&
            players[1].getRemainingPokemon() > 1
          ) {
        ...
        if (
          players[0].getRemainingPokemon() > players[1].getRemainingPokemon()
        ) {
          result = "win";
        } else {
          result = "loss";
  ...
  this.simulate = function() {
    ...
    battleRatings = [
      pokemon[0].getBattleRating(),
      pokemon[1].getBattleRating()
    ];
  ...
  this.emulate = function(callback) {
    ...
    players[1]
      .getAI()
      .evaluateMatchup(turns, pokemon[1], pokemon[0], players[0]);
  ...
  this.getTurnAction = function(poke, opponent) {
    ...
    if (poke.cooldown == 0 && !poke.hasActed) {
      if (
        !sandbox ||
        (mode == "emulate" &&
          players[poke.index].getAI() !== false &&
          poke.hp > 0)
      ) {
        poke.hasActed = true;
        ...
        if (mode == "simulate") {
          ...
          action = players[poke.index]
            .getAI()
            .decideAction(turns, poke, opponent);
        }
      } else {
        ...
        for (var n = 0; n < actions.length; n++) {
          ...
          if (
            (mode == "simulate" &&
              a.actor == poke.index &&
              a.turn == turns &&
              poke.chargedMoves.length > a.value) ||
            ...
            if (
              action.type == "charged" &&
              poke.energy < poke.chargedMoves[action.value].energy
            ) {
              action = null;
            }
      ...
      if (
        !action &&
        (mode == "simulate" ||
          (mode == "emulate" && players[poke.index].getAI() !== false))
      ) {
      ...
      if (action && action.type == "fast") {
        timeline.push(
          new TimelineEvent("tap interaction", "Tap", poke.index, time, turns, [
            2,
            0
          ])
        );
  ...
  this.decideAction = function(poke, opponent) {
    ...
    if (poke.bestChargedMove && poke.energy >= poke.bestChargedMove.energy) {
      ...
      self.logDecision(
        turns,
        poke,
        "'s best charged move is charged (" + poke.bestChargedMove.name + ")"
      );
      ...
      if (opponent.hp <= faintThreshold) {
        ...
        self.logDecision(
          ...
          " doesn't use " +
            ...
            " because a fast move will knock out the opponent"
        );
      }
      ...
      if (opponent.shields > 0) {
        if (
          opponent.hp <=
          poke.fastMove.damage *
            (opponent.fastMove.cooldown / poke.fastMove.cooldown)
        ) {
          useChargedMove = false;
          ...
          self.logDecision(
            ...
            " doesn't use " +
              poke.bestChargedMove.name +
              " because opponent has shields and fast moves will knock them out before their cooldown completes"
          );
        }
      }
      ...
      for (var n = 0; n < poke.chargedMoves.length; n++) {
        ...
        if (
          poke.energy >= poke.chargedMoves[n].energy &&
          (poke.chargedMoves[n].energy < poke.bestChargedMove.energy ||
            (poke.chargedMoves[n].energy == poke.bestChargedMove.energy &&
              poke.chargedMoves[n].buffs))
        ) {
          if (
            poke.baitShields &&
            opponent.shields > 0 &&
            !poke.chargedMoves[n].selfDebuffing
          ) {
            useChargedMove = false;
            ...
            self.logDecision(
              ...
              " doesn't use " +
                poke.bestChargedMove.name +
                " because it has a cheaper move to remove shields"
            );
          }
          ...
          if (opponent.hp <= poke.chargedMoves[n].damage) {
            ...
            self.logDecision(
              ...
              " doesn't use " +
                poke.bestChargedMove.name +
                " because it has a cheaper move to faint"
            );
          }
        }
      }
      ...
      if (
        poke.bestChargedMove.buffTarget &&
        poke.bestChargedMove.buffTarget == "self" &&
          (poke.bestChargedMove.buffs[0] < 0 ||
            poke.bestChargedMove.buffs[1] < 0) &&
        poke.energy < Math.min(poke.bestChargedMove.energy * 2, 100)
      ) {
        useChargedMove = false;
        ...
        self.logDecision(
          ...
          " doesn't use " +
            poke.bestChargedMove.name +
            " because it wants to minimize time debuffed"
        );
      }
    ...
    for (var n = 0; n < poke.activeChargedMoves.length; n++) {
      ...
      if (poke.energy >= move.energy && !chargedMoveUsed) {
        ...
        if (
          move.damage >= opponent.hp &&
          !poke.farmEnergy &&
          move.energy <= poke.bestChargedMove.energy &&
          !chargedMoveUsed
        ) {
          ...
          if (
            ...
            poke.energy < Math.min(move.energy * 2, 100)
          ) {
          ...
          if (
            ...
            (poke.bestChargedMove.buffs[0] < 0 ||
              poke.bestChargedMove.buffs[1] < 0) &&
            poke.bestChargedMove.energy < Math.min(move.energy * 2, 100)
          ) {
            avoidSelfDebuff = true;
          }
          ...
          if (!avoidSelfDebuff) {
            ...
            self.logDecision(
              turns,
              poke,
              " will knock out opponent with " + move.name
            );
            return action;
          }
        }
        ...
        if (
          ...
          poke.fastMove.damage /
            ((poke.fastMove.cooldown / 500) * opponent.stats.hp) >=
            0.025 &&
          opponent.hp > poke.bestChargedMove.damage &&
          (move.buffs[0] > 0 || move.buffs[1] > 0)
        ) {
          action = new TimelineAction("charged", poke.index, turns, moveIndex, {
            shielded: false,
            buffs: false,
            priority: poke.priority
          });
          ...
          self.logDecision(
            turns,
            poke,
            " is looking to build up stat boosts with " + move.name
          );
          return action;
        }
        ...
        if (
          ...
          !chargedMoveUsed &&
          !poke.farmEnergy &&
          ((!poke.baitShields && move == poke.bestChargedMove) ||
            (poke.baitShields &&
              poke.energy >= poke.bestChargedMove.energy &&
              move == poke.fastestChargedMove))
        ) {
          ...
          if (
            ...
            if (!avoidSelfDebuff) {
              self.logDecision(
                turns,
                poke,
                " wants to remove shields with " +
                  move.name +
                  " and opponent won't faint from fast move damage before next cooldown"
              );
        ...
        if (
          ...
          opponent.cooldown / poke.fastMove.cooldown < 3
        ) {
          nearDeath = true;
          ...
          self.logDecision(
            turns,
            poke,
            " will be knocked out by opponent's fast move this turn"
          );
        }
        ...
        if (
          ...
          opponent.fastMove.cooldown == 500
        ) {
          nearDeath = true;
          ...
          self.logDecision(
            turns,
            poke,
            " will be knocked out by opponent's fast move next turn"
          );
        }
        ...
        if (poke.shields == 0) {
          for (var j = 0; j < opponent.chargedMoves.length; j++) {
            if (
              opponent.energy >= opponent.chargedMoves[j].energy &&
              poke.hp <=
                self.calculateDamage(opponent, poke, opponent.chargedMoves[j])
            ) {
              nearDeath = true;
              ...
              self.logDecision(
                turns,
                poke,
                " doesn't have shields and will by knocked out by opponent's " +
                  opponent.chargedMoves[j].name +
                  " this turn"
              );
            }
          }
        ...
        if (
          ((opponent.cooldown > 0 &&
            opponent.cooldown < poke.fastMove.cooldown) ||
            (opponent.cooldown == 0 &&
              opponent.fastMove.cooldown < poke.fastMove.cooldown)) &&
          roundChargedMoveUsed == 0
        ) {
          ...
          var futureActions = Math.ceil(
            availableTime / opponent.fastMove.cooldown
          );
          ...
          if (poke.hp <= futureFastDamage) {
            ...
            self.logDecision(
              turns,
              poke,
              " will be knocked out by future fast move damage"
            );
          }

          // Can this Pokemon be knocked out by future Charged Moves
          if (poke.shields == 0) {
            var futureEffectiveEnergy =
              opponent.energy +
              opponent.fastMove.energyGain * (futureActions - 1);
            var futureEffectiveHP =
              poke.hp - (futureActions - 1) * opponent.fastMove.damage;
            ...
            for (var j = 0; j < opponent.chargedMoves.length; j++) {
              if (
                futureEffectiveEnergy >= opponent.chargedMoves[j].energy &&
                futureEffectiveHP <= opponent.chargedMoves[j].damage
              ) {
                nearDeath = true;
                ...
                self.logDecision(
                  turns,
                  poke,
                  " doesn't have shields and will be knocked out by future fast and charged move damage"
                );
              }
            }
          }
        ...
        if (opponent.shields > 0 && opponent.hp <= poke.fastMove.damage) {
          ...
          self.logDecision(
            ...
            " doesn't use " +
              move.name +
              " because opponent has shields and will faint from a fast move this turn"
          );
        }
        ...
        if (
          ...
          poke.energy >= poke.bestChargedMove.energy &&
          move.damage < poke.bestChargedMove.damage &&
          (!poke.baitShields || opponent.shields == 0)
        ) {
          nearDeath = false;
          ...
          self.logDecision(
            turns,
            poke,
            " doesn't use " + move.name + " because a better move is available"
          );
        }
  ...
  this.queueAction = function(actor, type, value) {
    ...
    var action = new TimelineAction(type, actor, turns, value, {
      shielded: false,
      buffs: false,
    ...
    if (type == "switch" && phase == "suspend_switch") {
      ...
      for (var i = 0; i < phaseProps.actors.length; i++) {
        for (var n = 0; n < actions.length; n++) {
          if (
            actions[n].type == "switch" &&
            actions[n].actor == phaseProps.actors[i]
          ) {
            switchesAnswered++;

            // Reset the incoming Pokemon's buffs and debuffs
            players[actions[n].actor].getTeam()[actions[n].value].statBuffs = [
              0,
              0
            ];
            players[actions[n].actor].getTeam()[
              actions[n].value
            ].startStatBuffs = [0, 0];
          }
        }
        ...
        if (poke.energy >= move.energy) {
          if (mode == "simulate") {
            self.useMove(
              ...
              opponent,
              move,
              action.settings.shielded,
              action.settings.buffs,
              action.settings.charge
            );
            ...
            for (var i = 0; i < turnActions.length; i++) {
              if (
                turnActions[i].type == "charged" &&
                turnActions[i].actor != poke.index
              ) {
            ...
            if (players[opponent.index].getAI() !== false) {
              playerUseShield = players[opponent.index]
                .getAI()
                .decideShield(poke, opponent, move);
            }
  ...
  this.useMove = function(
    ...
    if (move.energy > 0) {
      ...
      if (
        (sandbox && forceShields && defender.shields > 0) ||
        (!sandbox && defender.shields > 0)
      ) {
        var useShield = true;
        ...
        if (
          ...
          var fastDamage = self.calculateDamage(
            attacker,
            defender,
            attacker.fastMove
          );
          ...
          var fastAttacks =
            Math.ceil(
              Math.max(move.energy - attacker.energy, 0) /
                attacker.fastMove.energyGain
            ) + 2; // Give some margin for error here
          ...
          for (var i = 0; i < attacker.chargedMoves.length; i++) {
            ...
            if (attacker.energy + chargedMove.energy >= chargedMove.energy) {
              var chargedDamage = self.calculateDamage(
                attacker,
                defender,
                chargedMove
              );
        ...
        if (useShield) {
          ...
          self.logDecision(
            turns,
            defender,
            " doesn't shield because it can withstand the attack and is saving shields for later, boosted attacks"
          );
        }
      } else {
        // No shield used
      ...
      if (mode == "emulate") {
        attacker.battleStats.energyGained += Math.min(
          move.energyGain,
          100 - attacker.energy
        );
      }
    ...
    if (mode == "emulate") {
      attacker.battleStats.damage +=
        (Math.min(damage, defender.hp) / defender.stats.hp) * 100;

      if (attacker.battleStats.shieldsUsed > 0) {
        attacker.battleStats.damageFromShields +=
          (Math.min(damage, defender.hp) / defender.stats.hp) * 100;
      }
    }
          ...
          for (var i = move.buffs.length - 1; i >= 0; i--) {
            if (move.buffs[i] != 0) {
              turnMessages.push({
                ...
                str:
                  statNames[i] + " " + statDescriptions[move.buffs[i] + 2] + "!"
              });
            }
          }
        }
    ...
    if (!buffApplied) {
      timeline.push(
        new TimelineEvent(type, move.name, attacker.index, displayTime, turns, [
          damage,
          energyValue
        ])
      );
  ...
  this.convertTimelineToActions = function() {
    ...
    for (var i = 0; i < timeline.length; i++) {
      ...
      if (event.type.indexOf("charged") > -1) {
        ...
        if (
          timeline[i - 1] &&
          timeline[i - 1].type == "shield" &&
          timeline[i - 1].actor != event.actor
        ) {
          shielded = true;
        }
        ...
        actions.push(
          new TimelineAction("charged", event.actor, event.turn, index, {
            shielded: shielded,
            buffs: buffs,
            charge: 1,
