...
var file = webRoot + "data/training/teams/featured/featured-december.json?v=9";
var featuredTeams = [];
...
$.getJSON(file, function(data) {
  ...
  for (var i = 0; i < featuredTeams.length; i++) {
    $(".featured-team-select").append(
      '<option value="' +
        featuredTeams[i].slug +
        '">' +
        featuredTeams[i].name +
        " (" +
        featuredTeams[i].cupName +
        ")</option>"
    );
  }
});
  ...
  function createInstance(matchHandler) {
    ...
    function interfaceObject(matchHandler) {
      ...
      this.openTeamSelect = function(players, roundRecord) {
        ...
        for (var i = 0; i < players.length; i++) {
          ...
          for (var n = 0; n < roster.length; n++) {
            ...
            if (i == 0) {
              $el
                .find(".sprite-container")
                .append('<div class="number"></div>');
            }
      ...
      function startBattle(e) {
        ...
        if (
          teams[0].length < partySize ||
          (teamSelectMethod == "manual" && teams[1].length < partySize)
        ) {
        ...
        if (teamSelectMethod == "featured" && !featuredTeam) {
          modalWindow(
            "Featured Team",
            $("<p>Please select a featured team to fight.</p>")
          );
      ...
      function selectFeaturedTeam(e) {
        ...
        for (var i = 0; i < featuredTeams.length; i++) {
          ...
          if (team.slug == slug) {
            $(".featured-team-description img").attr(
              "src",
              webRoot + "img/train/featured/" + team.img + ".png?v=2"
            );
            ...
            $(
              '.league-cup-select option[value="' +
                team.league +
                " " +
                team.cup +
                '"]'
      ...
      function selectRosterPokemon(e) {
        ...
        if (!$el.hasClass("selected")) {
          ...
          $(
            ".self .roster .pokemon[team-index=" + currentTeamIndex + "]"
          ...
          for (
            currentTeamIndex = 0;
            currentTeamIndex < currentTeam.length;
            currentTeamIndex++
          ) {
      ...
      function selectLeague(e) {
        var vals = $(".league-cup-select option:selected")
          .val()
          .split(" ");
        var cp = vals[0];
        var cup = vals[1];
        ...
        if (cup == "cliffhanger") {
          $('.team-method-select option[value="featured"]').prop(
            "selected",
            "selected"
          );
