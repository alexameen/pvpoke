...
function TrainingAI(l, p, b) {
  ...
  this.generateRoster = function(size, callback) {
    ...
    for (var i = 0; i < slots.length; i++) {
      ...
      var ivCombos = pokemon.generateIVCombinations(
        "overall",
        1,
        props.ivComboRange
      );
  ...
  this.generateTeam = function(
    ...
    if (!previousResult) {
      ...
      pickStrategyOptions.push(
        new DecisionOption("SAME_TEAM_DIFFERENT_LEAD", winStratWeight)
      );
      pickStrategyOptions.push(
        new DecisionOption("COUNTER_LAST_LEAD", loseStratWeight)
      );
    ...
    switch (pickStrategy) {
      ...
      case "BEST":
        ...
        teamPerformance = self.calculateAverageRosterPerformance(
          roster,
          targets
        );
      ...
      case "COUNTER":
        ...
        // Next, let's give it a bodyguard
        var scenarios = self.runBulkScenarios(
          "NO_BAIT",
          team[0],
          opponentRoster
        );
  ...
  this.evaluateMatchup = function(turn, pokemon, opponent, opponentPlayer) {
    ...
    var overallRating =
      (scenarios.bothBait.average +
        scenarios.neitherBait.average +
        scenarios.noBait.average) /
      3;
    var options = [];
    ...
    if (
      ...
      var switchWeight = Math.floor(
        Math.max((switchThreshold - overallRating) / 10, 0)
      );
      ...
      if (self.hasStrategy("SWITCH_FARM")) {
        ...
        var weightFactor = Math.pow(
          Math.round(Math.max(3 - percentPerTurn, 0)),
          2
        );
        ...
        if (
          opponent.fastMove.energyGain / (opponent.fastMove.cooldown / 500) <
          3
        ) {
          weightFactor = 0;
        }
        ...
        if (
          pokemon.hp / pokemon.stats.hp < 0.5 &&
          opponentPlayer.getRemainingPokemon() > 1
        ) {
          weightFactor = 0.1;
        }
        ...
        options.push(
          new DecisionOption(
            "SWITCH_FARM",
            Math.floor(switchWeight * weightFactor)
          )
        );
      }
    }
    ...
    if (totalSwitchWeight < 10) {
      ...
      if (
        ...
        var baitWeight = Math.max(
          Math.round(
            (scenarios.bothBait.average - scenarios.noBait.average) / 20
          ),
          1
        );
        ...
        if (
          scenarios.bothBait.average >= 500 &&
          pokemon.chargedMoves.length == 2
        ) {
          if (
            Math.abs(
              pokemon.chargedMoves[0].dpe - pokemon.chargedMoves[1].dpe
            ) <= 0.1
          ) {
            baitWeight += 5;
          }
        ...
        if (
          pokemon.bestChargedMove.moveId == "SKULL_BASH" &&
          player.getRemainingPokemon() > 1
        ) {
          baitWeight = 0;
        }
  ...
  this.runScenario = function(type, pokemon, opponent) {
    ...
    var maxShields = Math.max(
      pokemon.startingShields,
      opponent.startingShields
    );
  ...
  this.decideAction = function(turn, poke, opponent) {
    ...
    var futureEnergy =
      opponent.energy + extraFastMoves * opponent.fastMove.energyGain;
    var futureDamage = self.calculatePotentialDamage(
      opponent,
      poke,
      futureEnergy
    );
    ...
    if (
      ...
      if (
        currentStrategy == "SWITCH_BASIC" &&
        turn - turnLastEvaluated >= props.reactionTime
      ) {
        performSwitch = true;
      }
    ...
    for (var i = 0; i < team.length; i++) {
      ...
      if (pokemon.hp > 0 && pokemon != poke) {
        ...
        if (scenario.average < 500) {
          weight = Math.max(
            Math.round(Math.pow(scenario.average / 100, 4) / 20),
            1
          );
        } else {
  ...
  this.decideShield = function(attacker, defender, m) {
    ...
    var estimatedEnergy =
      attacker.energy +
      (Math.floor(Math.random() * (props.energyGuessRange * 2)) -
        props.energyGuessRange);
    ...
    moves.sort((a, b) =>
      a.damage > b.damage ? -1 : b.damage > a.damage ? 1 : 0
    );
    ...
    var damageWeight = Math.min(
      Math.round(
        (move.damage / Math.max(defender.hp, defender.stats.hp / 2)) * 10
      ),
      10
    );
    ...
    for (var i = 0; i < defender.chargedMoves.length; i++) {
      ...
      var turnsAway =
        Math.ceil(
          (move.energy - defender.energy) / defender.fastMove.energyGain
        ) *
        (defender.fastMove.cooldown / 500);

      if (
        (moveDamage >= attacker.hp || moveDamage >= defender.stats.hp * 0.8) &&
        turnsAway <= 1
      ) {
  ...
  this.calculatePotentialDamage = function(attacker, defender, energy, stack) {
    ...
    for (var i = 0; i < attacker.chargedMoves.length; i++) {
      var countMultiplier = Math.floor(
        energy / attacker.chargedMoves[i].energy
      );
      if (!stack) {
        countMultiplier = 0;
