  ...
  function createInstance(matchHandler) {
    ...
    function interfaceObject(matchHandler) {
      ...
      this.init = function(props, b, p) {
        ...
        $(".battle-window .scene .pokemon-container").removeClass(
          "animate-switch"
        );
      ...
      this.update = function(response) {
        ...
        if (phase != response.phase) {
          ...
          switch (response.phase) {
            case "suspend_charged_attack":
              ...
              $(".charge-window .move-bars").append(
                $(".controls .move-bar")
                  .eq(response.move)
                  .clone()
                  .addClass(activePokemon[0].chargedMoves[response.move].type)
              );
              $(".battle-window .animate-message .text").html(
                "Tap to charge up " +
                  activePokemon[0].chargedMoves[response.move].name +
                  "!"
              );
              $(".switch-window").removeClass("active");
            ...
            case "animating":
              $(".animate-message .text").html(
                activePokemon[response.actor].speciesName +
                  " used " +
                  response.moveName
              );
              ...
              if (phase == "suspend_charged_attack") {
                var chargeMultiplier = Math.min(
                  0.25 + 0.75 * (charge / maxCharge),
                  1
                );
            ...
            case "game_over":
              ...
              switch (response.result) {
                ...
                case "loss":
                  ...
                  $(".battle-window .end-screen .subtitle").html(
          ...
          switch (phase) {
            case "animating":
              $(
                ".battle-window .pokemon-container .shield-sprite-container"
              ).removeClass("active");
              break;
          }
        }
        ...
        for (var i = 0; i < response.pokemon.length; i++) {
          ...
          if (activePokemon.length < i || activePokemon[i] != pokemon) {
            ...
            if (i == 0) {
              ...
              for (var n = 0; n < pokemon.chargedMoves.length; n++) {
                ...
                $bar
                  .find(".bar-back")
                  .attr("class", "bar-back " + chargedMove.type);
                $(".battle-window .move-labels .label")
                  .eq(n)
                  .html(chargedMove.name);
              }
            }
          }
          ...
          if (i == 0) {
            for (var n = 0; n < pokemon.chargedMoves.length; n++) {
              var chargedMove = pokemon.chargedMoves[n];
              var $bar = $(".battle-window .move-bar").eq(n);
              var chargePercent = Math.min(
                pokemon.energy / chargedMove.energy,
                1
              );
              ...
              $bar
                .find(".bar")
                .first()
                .css("background-position-y", position + "px");
        ...
        for (var i = 0; i < response.players.length; i++) {
          ...
          $(".battle-window .shields")
            .eq(i)
            .html(player.getShields());
          ...
          for (var j = 0; j < activePokemonCount; j++) {
            if (i == 0) {
              $(".team-indicator.left .ball")
                .eq(j)
                .removeClass("fainted");
            } else {
              $(".team-indicator.right .ball")
                .eq(2 - j)
                .removeClass("fainted");
            }
          }
          ...
          if (i == 0) {
            if (player.getSwitchTimer() == 0) {
              ...
              $(".battle-window .switch-btn").removeClass("active");
              $(".battle-window .switch-btn").html(
                Math.floor(player.getSwitchTimer() / 1000)
              );
        ...
        switch (phase) {
          case "countdown":
            if (
              response.countdown !== undefined &&
              countdown + 1 != response.countdown
            ) {
        ...
        for (var i = 0; i < response.messages.length; i++) {
          ...
          var $messageItem = $(
            '<div turn="' + response.turn + '">' + message.str + "</div>"
          );
          ...
          if (message.index == 1 && message.str == "Blocked!") {
            $(
              ".battle-window .pokemon-container.opponent .shield-sprite-container"
            ).addClass("active");
          }
        }
        ...
        for (var i = 0; i < response.animations.length; i++) {
          ...
          switch (animation.type) {
            case "fast":
              ...
              $(".battle-window .scene .pokemon-container")
                .eq(animation.actor)
                .find(".fast-move-circle")
                .attr(
                  "class",
                  "fast-move-circle " +
                    activePokemon[animation.actor].fastMove.type
                );
              $(".battle-window .scene .pokemon-container")
                .eq(animation.actor)
            ...
            case "switch":
              ...
              $(".battle-window .scene .pokemon-container")
                .eq(animation.actor)
                .addClass("animate-switch");
              $(".battle-window .scene .pokemon-container")
                .eq(animation.actor)
                .attr("animation-time", time);
              ...
              if (animation.value === false) {
                ...
                $(".battle-window .scene .pokemon-container")
                  .eq(animation.actor)
                  .attr("animation-duration", duration);
              }
        ...
        for (var i = 0; i < team.length; i++) {
          if (team[i] != activePokemon[0]) {
            ...
            $(".switch-window .pokemon")
              .eq(index)
              .find(".name")
              .html(team[i].speciesName);
            $(".switch-window .pokemon")
              .eq(index)
              .find(".name")
              .attr("class", "name " + team[i].types[0]);
            $(".switch-window .pokemon")
              .eq(index)
              .attr("index", i);
            $(".switch-window .pokemon")
              .eq(index)
              .attr("type-1", team[i].types[0]);
            if (team[i].types[1] == "none") {
              $(".switch-window .pokemon")
                .eq(index)
                .attr("type-2", team[i].types[0]);
            } else {
              $(".switch-window .pokemon")
                .eq(index)
                .attr("type-2", team[i].types[1]);
            }
      ...
      self.completeSwitchAnimation = function() {
        $(".battle-window .pokemon-container.animate-switch").each(function(
          index,
          value
        ) {
          ...
          if (
            timePassed >= parseInt($(this).attr("animation-duration")) &&
            activePokemon[index].hp > 0
          ) {
            $(this).removeClass("animate-switch");
            $(this)
              .find(".pokemon")
              .attr("type-1", activePokemon[index].types[0]);
            $(this)
              .eq(index)
              .find(".name")
              .attr("class", "name " + activePokemon[index].types[0]);

            $(".team-indicator")
              .eq(index)
              .find(".name")
              .html(activePokemon[index].speciesName);
            $(".team-indicator")
              .eq(index)
              .find(".cp")
              .html(activePokemon[index].cp);

            if (activePokemon[index].types[1] != "none") {
              $(this)
                .find(".pokemon")
                .attr("type-2", activePokemon[index].types[1]);
            } else {
              $(this)
                .find(".pokemon")
                .attr("type-2", activePokemon[index].types[0]);
            }
          }
        });
      };
      ...
      self.displayEndGameStats = function() {
        ...
        for (var k = 0; k < players.length; k++) {
          ...
          for (var i = 0; i < team.length; i++) {
            ...
            $(".battle-stats .damage-section")
              .eq(k)
              .find(".pokemon-entry .name")
              .eq(i)
              .html(pokemon.speciesName);
            $(".battle-stats .damage-section")
              .eq(k)
              .find(".pokemon-entry .damage-bar")
              .eq(i)
              .css("width", width + "%");
            $(".battle-stats .damage-section")
              .eq(k)
        ...
        for (var i = 0; i < team.length; i++) {
          ...
          damageFromShields += Math.round(
            pokemon.battleStats.damageFromShields
          );
          shieldsFromShields += pokemon.battleStats.shieldsFromShields;
        }

        $(".battle-stats .tab-section.shields .stat-shields-used").html(
          shieldsUsed
        );
        $(".battle-stats .tab-section.shields .stat-damage-dealt").html(
          Math.round((damageFromShields / totalDamage) * 1000) / 10 + "%"
        );
        $(".battle-stats .tab-section.shields .stat-shields-drawn").html(
          shieldsFromShields
        );
        $(".battle-stats .tab-section.shields .stat-damage-blocked").html(
          totalDamageBlocked
        );

        // Display energy stats

        $(".battle-stats .tab-section.energy .stat-energy-gained").html(
          totalEnergyGained
        );
        $(".battle-stats .tab-section.energy .stat-energy-used").html(
          totalEnergyUsed
        );
        $(".battle-stats .tab-section.energy .stat-energy-lost").html(
          totalEnergyLost
        );
        $(".battle-stats .tab-section.energy .stat-avg-dpe").html(
          Math.round((totalChargedDamage / totalEnergyUsed) * 100) / 100
        );
      ...
      this.reportBattleAnalytics = function(result) {
        var battleSummaryStr =
          battle.getCup().name +
          " " +
          battle.getCP() +
          " difficulty " +
          (players[1].getAI().getLevel() + 1);
        ...
        for (var i = 0; i < players.length; i++) {
          ...
          for (var n = 0; n < team.length; n++) {
            var pokemon = team[n];
            var pokeStr =
              pokemon.speciesName + " " + pokemon.fastMove.abbreviation;
            var chargedMoveAbbrevations = [];

            for (var k = 0; k < pokemon.chargedMoves.length; k++) {
              chargedMoveAbbrevations.push(
                pokemon.chargedMoves[k].abbreviation
              );
            }

            // Sort alphabetically
            chargedMoveAbbrevations.sort((a, b) =>
              a > b ? 1 : b > a ? -1 : 0
            );
            ...
            pokemon.battleStats.score = Math.round(
              pokemon.battleStats.damage +
                50 * pokemon.battleStats.shieldsBurned
            );
        ...
        for (var i = 0; i < players.length; i++) {
          ...
          var battleRating = Math.floor(
            500 * ((maxScore - opponentScore) / maxScore) +
              500 * (score / maxScore)
          );
      ...
      function battleWindowClick(e) {
        ...
        if ($(".battle-window .controls .button-stack:hover").length > 0) {
          ...
          if ($(".controls .button-stack .pause-btn:hover").length > 0) {
            ...
            battle.setPause(
              $(".controls .button-stack .pause-btn").hasClass("active")
            );
          }
        ...
        if (
          $(".battle-window .controls .move-bar.active:hover").length == 0 &&
          $(".battle-window .switch-window:hover").length == 0
        ) {
          // Fast move
          battle.queueAction(0, "fast", 0);
        } else {
      ...
      function useShieldClick() {
        if (interfaceLockout == 0) {
          $(".battle-window .shield-window").addClass("closed");
          $(
            ".battle-window .pokemon-container.self .shield-sprite-container"
          ).addClass("active");
