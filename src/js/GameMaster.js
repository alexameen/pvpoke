...
var GameMaster = (function() {
  ...
  function createInstance(interface) {
    ...
    $.getJSON(webRoot + "data/gamemaster.json?v=202", function(data) {
      object.data = data;

      // Sort Pokemon alphabetically for searching
      object.data.pokemon.sort((a, b) =>
        a.speciesName > b.speciesName
          ? 1
          : b.speciesName > a.speciesName
          ? -1
          : 0
      );
      ...
      $.each(object.data.pokemon, function(index, poke) {
        ...
        if (object.data.shadowPokemon.indexOf(poke.speciesId) > -1) {
          ...
          for (var i = 0; i < entry.chargedMoves.length; i++) {
            if (
              entry.chargedMoves[i] == "RETURN" ||
              entry.chargedMoves[i] == "FRUSTRATION"
            ) {
          ...
          if (entry.legacyMoves) {
            for (var i = 0; i < entry.legacyMoves.length; i++) {
              if (
                entry.legacyMoves[i] == "RETURN" ||
                entry.legacyMoves[i] == "FRUSTRATION"
              ) {
    ...
    object.generateDefaultIVs = function() {
      $.each(object.data.pokemon, function(index, poke) {
        ...
        for (var i = 0; i < leagues.length; i++) {
          ...
          var cp = pokemon.calculateCP(0.79030001, 15, 15, 15);

          if (cp > leagues[i]) {
            var combinations = pokemon.generateIVCombinations(
              "overall",
              1,
              4096
            );
            var defaultIndex = Math.floor(combinations.length * 0.12207);
    ...
    object.loadGroupData = function(caller, group) {
      ...
      if (!object.groups[key]) {
        var file = webRoot + "data/groups/" + group + ".json?v=202";

        $.getJSON(file, function(data) {
          // Sort alphabetically

          data.sort((a, b) =>
            a.speciesId > b.speciesId ? 1 : b.speciesId > a.speciesId ? -1 : 0
          );
    ...
    object.loadTeamData = function(league, cup, callback) {
      ...
      if (!object.teamPools[key]) {
        var file =
          webRoot + "data/training/teams/" + cup + "/" + league + ".json?v=202";
      ...
      $.each(object.data.pokemon, function(index, poke) {
        if (poke.speciesId == id) {
          switch (type) {
            case "movepool":
              var movepool =
                props.moveType == "fast" ? poke.fastMoves : poke.chargedMoves;
    ...
    object.generateFilteredPokemonList = function(
      battle,
      include,
      exclude,
      rankingData,
      ...
      for (var i = 0; i < object.data.pokemon.length; i++) {
        ...
        var stats =
          (pokemon.stats.hp * pokemon.stats.atk * pokemon.stats.def) / 1000;
        ...
        if (stats >= minStats) {
          ...
          for (var n = 0; n < filterLists.length; n++) {
            ...
            for (var j = 0; j < filters.length; j++) {
              ...
              switch (filter.filterType) {
                case "type":
                  if (
                    filter.values.indexOf(pokemon.types[0]) > -1 ||
                    filter.values.indexOf(pokemon.types[1]) > -1
                  ) {
                    filtersMatched++;
                  }
                  break;
                ...
                case "dex":
                  if (
                    pokemon.dex >= filter.values[0] &&
                    pokemon.dex <= filter.values[1]
                  ) {
                    filtersMatched++;
                  }
                  break;
            ...
            if (
              pokemon.dex > maxDexNumber &&
              releasedGen5.indexOf(pokemon.speciesId) == -1 &&
              battle.getCup().name != "gen-5"
            ) {
              allowed = false;
            }
          ...
          if (
            battle.getCP() == 1500 &&
            bannedList.indexOf(pokemon.speciesId) > -1
          ) {
            allowed = false;
          }
      ...
      for (var i = 0; i < queries.length; i++) {
        ...
        for (var n = 0; n < object.data.pokemon.length; n++) {
          var pokemon = new Pokemon(
            object.data.pokemon[n].speciesId,
            0,
            battle
          );
          var paramsMet = 0;
          ...
          for (var j = 0; j < params.length; j++) {
            ...
            if (param.charAt(0) == "@" && param.length > 2) {
              ...
              for (var k = 0; k < pokemon.fastMovePool.length; k++) {
                if (
                  pokemon.fastMovePool[k].name
                    .toLocaleLowerCase()
                    .startsWith(param) ||
                  pokemon.fastMovePool[k].type == param
                ) {
                  valid = true;
                }
              }
              ...
              for (var k = 0; k < pokemon.chargedMovePool.length; k++) {
                if (
                  pokemon.chargedMovePool[k].name
                    .toLocaleLowerCase()
                    .startsWith(param) ||
                  pokemon.chargedMovePool[k].type == param
                ) {
                  valid = true;
                }
              }
              ...
              if (
                pokemon.types.indexOf(param) > -1 ||
                pokemon.speciesName.toLowerCase().startsWith(param)
              ) {
                valid = true;
              }
            }
