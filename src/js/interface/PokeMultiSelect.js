...
function PokeMultiSelect(element) {
  ...
  this.init = function(pokes, b) {
    ...
    while (window.localStorage.key(i) !== null) {
      ...
      if (key !== "undefined" && key.indexOf("google") == -1) {
        $el
          .find(".quick-fill-select")
          .append(
            '<option value="' + key + '" type="custom">' + key + "</option>"
          );
      }
  ...
  this.saveCustomList = function(name, isNew) {
    ...
    if (!isNew) {
      modalWindow(
        "Custom Group Saved",
        $("<p><b>" + name + "</b> has been updated.</p>")
      );
    } else {
      // Add new group to all dropdowns

      $(".quick-fill-select").append(
        $('<option value="' + name + '" type="custom">' + name + "</option>")
      );
      $el
        .find(".quick-fill-select option")
        .last()
  ...
  $el.on("click", ".rankings-container .remove", function(e) {
    selectedIndex = $el
      .find(".rankings-container .rank")
      .index($(this).closest(".rank"));
  ...
  $el.find(".save-btn").click(function(e) {
    var selectedGroupType = $(
      ".quick-fill-select option[value='" + selectedGroup + "']"
    ).attr("type");
  ...
  $("body").on("click", ".modal .button.save", function(e) {
    ...
    if (
      $(".modal .save-list").attr("selector-index") ==
      $(".poke.multi").index($el)
    ) {
  ...
  $el.find(".shield-select").on("change", function(e) {
    settings.shields = parseInt(
      $el.find(".shield-select option:selected").val()
    );
  });
  ...
  this.selectGroup = function(id) {
    $el
      .find(".quick-fill-select option[value='" + id + "']")
      .prop("selected", "selected");
    $el.find(".quick-fill-select").trigger("change");
  };
}
