...
function PokeSelect(element, i) {
  ...
  this.update = function() {
    if (selectedPokemon) {
      ...
      $el
        .find(".attack .stat")
        .html(Math.floor(selectedPokemon.getEffectiveStat(0) * 10) / 10);
      $el
        .find(".defense .stat")
        .html(Math.floor(selectedPokemon.getEffectiveStat(1) * 10) / 10);
      ...
      var effectiveOverall = Math.round(
        (selectedPokemon.stats.hp *
          selectedPokemon.getEffectiveStat(0) *
          selectedPokemon.getEffectiveStat(1)) /
          1000
      );
      ...
      $el
        .find(".attack .bar")
        .css("width", selectedPokemon.stats.atk / 4 + "%");
      $el
        .find(".defense .bar")
        .css("width", selectedPokemon.stats.def / 4 + "%");
      $el
        .find(".stamina .bar")
        .css("width", selectedPokemon.stats.hp / 4 + "%");
      ...
      $el
        ...
        .css(
          "width",
          (selectedPokemon.hp / selectedPokemon.stats.hp) * 100 + "%"
        );
      $el
        .find(".hp .stat")
        .html(selectedPokemon.hp + " / " + selectedPokemon.stats.hp);

      $el.find(".types").html("");

      for (var i = 0; i < selectedPokemon.types.length; i++) {
        var typeStr =
          selectedPokemon.types[i].charAt(0).toUpperCase() +
          selectedPokemon.types[i].slice(1);
        if (selectedPokemon.types[i] != "none") {
          $el
            .find(".types")
            .append(
              '<div class="type ' +
                selectedPokemon.types[i] +
                '">' +
                typeStr +
                "</div>"
            );
      ...
      $fastSelect
        .find("option[value='" + selectedPokemon.fastMove.moveId + "']")
        .prop("selected", "selected");
      $fastSelect.attr(
        "class",
        "move-select fast " + selectedPokemon.fastMove.type
      );
      ...
      for (var i = 0; i < $el.find(".move-select.charged").length; i++) {
        if (i < selectedPokemon.chargedMoves.length) {
          ...
          $el
            ...
            .eq(i)
            .find("option[value='" + chargedMove.moveId + "']")
            .prop("selected", "selected");
          $el
            .find(".move-select.charged")
            .eq(i)
            .attr("class", "move-select charged " + chargedMove.type);
          ...
          $el
            ...
            .eq(i)
            .find(".bar")
            .attr("class", "bar " + chargedMove.type);
          $el
            .find(".move-bar")
            .eq(i)
            .find(".bar-back")
          ...
          $el
            .find(".move-select.charged")
            .eq(i)
            .attr("class", "move-select charged");
          $el
            .find(".move-select.charged")
            .eq(i)
            .find("option")
  ...
  this.animateHealth = function(amount) {
    ...
    $el
      .find(".hp .bar")
      .css("width", (health / selectedPokemon.stats.hp) * 100 + "%");
  ...
  this.animateEnergy = function(index, amount) {
    ...
    $bar
      ...
      .css(
        "height",
        (energy / selectedPokemon.chargedMoves[index].energy) * 100 + "%"
      );
  ...
  this.setSelectedPokemon = function(poke) {
    ...
    $el
      .find(
        "input.level, input.iv, input.stat-mod, input.start-hp, input.start-energy"
      )
      .val("");

    $pokeSelect
      .find('option[value="' + poke.speciesId + '"]')
      .prop("selected", "selected");
    ...
    $el
      .find('.shield-select option[value="' + poke.startingShields + '"]')
      .prop("selected", "selected");
  ...
  this.filterByTypes = function(types) {
    ...
    if (types.length > 0) {
      $pokeSelect.find("option").each(function(index, value) {
        if (
          types.indexOf($(this).attr("type-1")) > -1 ||
          types.indexOf($(this).attr("type-2")) > -1
        ) {
  ...
  $el.find(".move-select").on("change", function(e) {
    ...
    if (moveId != "custom") {
      ...
      for (var i = 0; i < gm.data.moves.length; i++) {
        ...
        if (
          ((isFastMove && move.energyGain > 0) ||
            (!isFastMove && move.energyGain == 0)) &&
          selectedPokemon.fastMovePool.indexOf(move.moveId) == -1 &&
            selectedPokemon.chargedMovePool.indexOf(move.moveId) == -1
        ) {
          var $option = $(
            '<option type="' +
              move.type +
      ...
      $(".modal")
        ...
        .on("click", function(e) {
          var moveId = $(".modal")
            .last()
            .find(".move-select option:selected")
            .val();
          var moveType = isFastMove ? "fast" : "charged";

          var pool = isFastMove
            ? selectedPokemon.fastMovePool
            : selectedPokemon.chargedMovePool;

          selectedPokemon.addNewMove(
            moveId,
            pool,
            true,
            moveType,
  ...
  $el.find(".maximize-stats").on("click", function(e) {
    var sortStat = $el
      .find(".maximize-section .check-group .check.on")
      .first()
      .attr("value");
    var levelCap = parseInt(
      $el
        .find(".maximize-section .level-cap-group .check.on")
        .first()
        .attr("value")
    );
  ...
  $el.on("mousemove", ".move-bar", function(e) {
    ...
    $tooltip
      .find(".details")
      .html(
        move.damage + " damage<br>" + move.energy + " energy<br>" + dpe + " dpe"
      );
