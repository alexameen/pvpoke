...
var InterfaceMaster = (function() {
  ...
  function createInstance() {
    ...
    function interfaceObject() {
      ...
      var chargeMultipliers = [1, 0.95, 0.75, 0.5, 0.25]; // Array of potential charge multipliers between full and minimum charge
      ...
      this.init = function() {
        ...
        $("body").on(
          "click",
          ".battle-details .rating-table a.rating.star",
          viewShieldBattle
        );
        $("body").on("click", ".section.summary a.rating.star", viewBulkBattle);
        $("body").on(
          "click",
          ".breakpoints-section .button",
          selectBreakpointIVs
        );
      ...
      this.displayRankingData = function(data) {
        ...
        if (self.battleMode == "multi") {
          ...
          $("html, body").animate(
            {
              scrollTop:
                $(".battle-results." + self.battleMode).offset().top - 185
            },
            500
          );
        }
      ...
      this.displayCumulativeDamage = function(timeline, time) {
        ...
        for (var i = 0; i < timeline.length; i++) {
          ...
          if (event.time <= time) {
            ...
            if (
              event.type.indexOf("fast") >= 0 ||
              event.type.indexOf("charged") >= 0
            ) {
              if (event.actor == 0) {
        ...
        for (var n = 0; n < pokeSelectors.length; n++) {
          ...
          cumulativeDamage[n] +=
            pokeSelectors[n].getPokemon().startHp - startingValues[n].hp;
          cumulativeEnergy[n] -=
            pokeSelectors[n].getPokemon().startEnergy -
            startingValues[n].energy;

          pokeSelectors[n].animateHealth(cumulativeDamage[n]);

          for (
            i = 0;
            i < pokeSelectors[n].getPokemon().chargedMoves.length;
            i++
          ) {
        ...
        if (timelineScaleMode == "fit") {
          ...
          if (animating) {
            if (left > $(".timeline-container").scrollLeft() - 100) {
              $(".timeline-container").scrollLeft(
                left - $(".timeline-container").width() + 100
              );
            }
      ...
      this.displayTimeline = function(b, bulkRatings, animate) {
        ...
        for (var i = 0; i < timeline.length; i++) {
          ...
          var $item = $(
            '<div class="item-container"><a href="#" class="item ' +
              event.type +
              '" index="' +
              i +
              '" actor="' +
          ...
          if (event.type.indexOf("interaction") > -1) {
            ...
            for (var n = 0; n < pokemon[event.actor].chargedMoves.length; n++) {
              if (
                energy[event.actor] >=
                pokemon[event.actor].chargedMoves[n].energy
              ) {
                usableChargedMoves++;
              }
            }
        ...
        if (timelineScaleMode == "fit") {
          ...
          var width = $(".timeline-container .item-container")
            .last()
            .position().left;
        ...
        if (winner.pokemon) {
          ...
          $(".battle-results .summary").html(
            '<span class="name">' +
              winner.pokemon.speciesName +
              '</span> wins in <span class="time">' +
              durationSeconds +
              's</span> with a battle rating of <span class="rating star">' +
              winnerRating +
              "</span>"
          );
          ...
          $(".battle-results .summary").html(
            'Simultaneous knockout in <span class="time">' +
              durationSeconds +
              "s</span>"
          );
        ...
        if (bulkRatings) {
          ...
          $(".battle-results .summary").append(
            '<div class="bulk-summary"></div>'
          );

          $(".battle-results .bulk-summary").append(
            '<div class="disclaimer">This matchup contains moves that have a chance to buff or debuff stats. These results are generated from 500 simulations, and may vary.</div>'
          );
          ...
          $(".battle-results .bulk-summary").append(
            "<p>" +
              pokemon.speciesName +
              '\'s best battle rating is <a href="#" class="rating star best">' +
              bestRating +
              "</a></p>"
          );
          $(".battle-results .bulk-summary").append(
            "<p>" +
              pokemon.speciesName +
              '\'s median battle rating is <a href="#" class="rating star median">' +
              medianRating +
              "</a></p>"
          );
          $(".battle-results .bulk-summary").append(
            "<p>" +
              pokemon.speciesName +
              '\'s worst battle rating is <a href="#" class="rating star worst">' +
              worstRating +
              "</a></p>"
          );
          ...
          bulkHistogram = new BattleHistogram(
            $(".battle-results .bulk-summary .histogram")
          );
          bulkHistogram.generate(
            pokeSelectors[0].getPokemon(),
            bulkRatings,
            400
          );
        }
      ...
      this.generateSingleBattleLinkString = function(sandbox) {
        ...
        battleStr +=
          ...
          pokeStrs[1] +
          "/" +
          pokes[0].startingShields +
          pokes[1].startingShields +
          "/" +
          moveStrs[0] +
          "/" +
          moveStrs[1] +
        ...
        for (var i = 0; i < actions.length; i++) {
          if (actions[i].valid) {
            ...
            str +=
              ...
              actions[i].actor +
              actions[i].value +
              (actions[i].settings.shielded ? 1 : 0) +
              (actions[i].settings.buffs ? 1 : 0) +
              (actions[i].settings.charge
                ? chargeMultipliers.indexOf(actions[i].settings.charge)
                : 0);
      ...
      this.generateMatchupDetails = function(battle, doBulk) {
        ...
        $(".rating-table .name-1.name").html(
          pokemon[0].speciesName.charAt(0) + "."
        );
        $(".battle-details .name-2").html(pokemon[1].speciesName);

        if (!sandbox) {
          var originalShields = [
            pokemon[0].startingShields,
            pokemon[1].startingShields
          ];
        ...
        for (var i = 0; i < 2; i++) {
          $(".stats-table .stat-total-damage")
            .eq(i)
            .html(totalDamage[i]);
          $(".stats-table .stat-damage-blocked")
            .eq(i)
            .html(damageBlocked[i]);

          var fastPercentage =
            Math.floor((fastDamage[i] / totalDamage[i]) * 1000) / 10;
          var chargedPercentage =
            Math.floor((chargedDamage[i] / totalDamage[i]) * 1000) / 10;

          $(".stats-table .stat-fast-damage")
            .eq(i)
            .html(fastDamage[i] + " (" + fastPercentage + "%)");
          $(".stats-table .stat-charged-damage")
            .eq(i)
            .html(chargedDamage[i] + " (" + chargedPercentage + "%)");

          $(".stats-table .stat-energy-gained")
            .eq(i)
            .html(energyGained[i]);
          $(".stats-table .stat-energy-used")
            .eq(i)
            .html(energyUsed[i]);
          ...
          $(".stats-table .stat-energy-remaining")
            ...
            .html(
              Math.min(
                energyGained[i] - energyUsed[i] + pokemon[i].startEnergy,
                100
              )
            );
          ...
          if (turnsToChargedMove[i] > 0) {
            $(".stats-table .stat-charged-time")
              .eq(i)
              .html(
                turnsToChargedMove[i] +
                  " (" +
                  turnsToChargedMove[i] * 0.5 +
                  "s)"
              );
          }
        }
        ...
        $(".stats-table.breakpoints .name-fast").html(
          pokemon[0].fastMove.name + " Damage"
        );
        $(".stats-table.breakpoints .output").html("<tr></tr>");

        for (var i = breakpoints.length - 1; i >= 0; i--) {
          var attack = Math.round(breakpoints[i].attack * 100) / 100;
          var guaranteedAttack =
            Math.round(breakpoints[i].guaranteedAttack * 100) / 100;
          ...
          var combinations = pokemon[0].generateIVCombinations(
            "overall",
            1,
            2,
          ...
          $(".stats-table.breakpoints .output").append(
            '<tr class="toggle"><td>' +
              breakpoints[i].damage +
              "</td><td>" +
              attack +
              "</td><td>" +
              guaranteedAttack +
              '</td><td class="ivs"><div class="button" level="' +
              combinations[0].level +
              '" atk="' +
              combinations[0].ivs.atk +
              '" def="' +
              combinations[0].ivs.def +
              '" hp="' +
              combinations[0].ivs.hp +
              '">' +
              combinations[0].level +
              " " +
              combinations[0].ivs.atk +
          ...
          if (breakpoints[i].damage == pokemon[0].fastMove.damage) {
            $(".stats-table.breakpoints .output tr")
              .last()
              .addClass("bold");
          }
        }
        ...
        $(".stats-table.bulkpoints .name-fast").html(
          pokemon[1].fastMove.name + " Damage"
        );
        ...
        for (var i = 0; i < bulkpoints.length; i++) {
          ...
          var guaranteedDefense =
            Math.round(bulkpoints[i].guaranteedDefense * 100) / 100;
          ...
          if (bulkpoints[i].damage == pokemon[1].fastMove.damage) {
            $(".stats-table.bulkpoints .output tr")
              .last()
              .addClass("bold");
          }
        }
        ...
        if (
          ...
          $(".breakpoints-section .golden-combination").append(
            '<p><div class="button" level="' +
              ...
              combinations[0].ivs.hp +
              "</div> " +
              pokemon[0].speciesName +
              " reaches the best breakpoint and bulkpoint against this " +
              pokemon[1].speciesName +
              ".</p>"
          );
        }
      };
      ...
      this.generateMultiBattleResults = function() {
        ...
        var opponentShields = parseInt(
          $(".poke.multi")
            .eq(0)
            .find(".shield-select option:selected")
            .val()
        );
        var chargedMoveCount = parseInt(
          $(".poke.multi .charged-count-select option:selected").val()
        );
        var shieldBaiting = $(".poke.multi")
          .eq(0)
          .find(".check.shield-baiting")
          .hasClass("on")
          ? 1
          : 0;
        ...
        ranker.setShieldBaitOverride(
          $(".poke.multi .check.shield-baiting").hasClass("on"),
          1
        );
        ...
        if (multiBattleWorstToBest) {
          rankings.sort((a, b) =>
            a.opRating > b.opRating ? 1 : b.opRating > a.opRating ? -1 : 0
          );
        }
        ...
        for (var i = 0; i < rankings.length; i++) {
          ...
          if (r.moveset) {
            ...
            for (var n = 0; n < r.moveset.chargedMoves.length; n++) {
              pokemon.selectMove(
                "charged",
                r.moveset.chargedMoves[n].moveId,
                n
              );
          ...
          var $el = $(
            '<div class="rank ' +
              ...
              '" data="' +
              pokemon.speciesId +
              '"><div class="name-container"><span class="number">#' +
              (i + 1) +
              '</span><span class="name">' +
              pokemon.speciesName +
              '</span></div><div class="rating-container"><div class="rating star">' +
              r.opRating +
              '</span></div><a target="_blank" href="' +
              battleLink +
              '"></a><div class="clear"></div></div><div class="details"></div>'
          );
          ...
          if (r.moveset) {
            $el
              .find(".name-container")
              .append('<div class="moves">' + moveNameStr + "</div>");
          }
        ...
        if (!histogram) {
          histogram = new BattleHistogram(
            $(".battle-results.multi .histogram")
          );
        ...
        var filename =
          ...
          moveAbbreviationStr +
          " vs " +
          $(".poke.multi .cup-select option:selected").html() +
          " " +
          poke.startingShields +
          "-" +
          opponentShields +
      ...
      this.generateMatrixResults = function() {
        ...
        for (var i = 0; i < rankings.length; i++) {
          ...
          for (var n = 0; n < r.matchups.length; n++) {
            var $cell = $(
              '<td><a class="rating star" href="#" target="blank"><span></span></a></td>'
            );
            ...
            var battleLink =
              ...
              "" +
              r.matchups[n].opponent.startingShields +
              "/" +
              moveStr +
              "/" +
              opMoveStr +
        ...
        // Update download link with new data
        var filename =
          multiSelectors[0].getSelectedGroup() +
          " vs " +
          multiSelectors[1].getSelectedGroup() +
          ".csv";
        var filedata = "";
        ...
        for (var key in get) {
          if (get.hasOwnProperty(key)) {
            ...
            switch (key) {
              ...
              case "cp":
                $('.league-select option[value="' + val + '"]').prop(
                  "selected",
                  "selected"
                );
                      ...
                      case "1":
                        ...
                        actions.push(
                          new TimelineAction(
                            ...
                            {
                              shielded:
                                parseInt(paramsArr[3]) == 1 ? true : false,
              ...
              case "mode":
                $('.mode-select option[value="' + val + '"]').prop(
                  "selected",
                  "selected"
                );
              ...
              case "cup":
                $('.cup-select option[value="' + val + '"]').prop(
                  "selected",
                  "selected"
                );

                if ($('.format-select option[cup="' + val + '"]').length > 0) {
                  $('.format-select option[cup="' + val + '"]').prop(
                    "selected",
                    "selected"
                  );
                } else {
                  var cat = $('.cup-select option[value="' + val + '"]').attr(
                    "cat"
                  );
                  $('.format-select option[value="' + cat + '"]').prop(
                    "selected",
                    "selected"
                  );
              ...
              case "cms":
                ...
                $('.charged-count-select option[value="' + arr[0] + '"]').prop(
                  "selected",
                  "selected"
                );
      ...
      function continueBattle(e) {
        ...
        $("html, body").animate(
          { scrollTop: $(".poke.single").offset().top - 25 },
          500
        );
      }
      ...
      function timelineEventHover(e) {
        ...
        if (
          ($(this).hasClass("fast") || $(this).hasClass("charged")) &&
          !$(this).hasClass("tap")
        ) {
          var values = $(this)
            .attr("values")
          ...
          $tooltip
            .find(".details")
            .html(values[0] + " damage<br>" + values[1] + " energy");
      ...
      function mainMouseMove(e) {
        ...
        if ($(".timeline-container:hover").length > 0 && !animating) {
          var offsetX =
            ($(window).width() - $(".timeline-container").width()) / 2;
          var posX = e.clientX - offsetX;
          var hoverTime;

          if (timelineScaleMode == "fit") {
            hoverTime =
              (battle.getDuration() + 2000) *
                (posX / $(".timeline-container").width()) -
              1000;
          } else if (timelineScaleMode == "zoom") {
            hoverTime =
              ((posX - 50 + $(".timeline-container").scrollLeft()) / 50) * 1000;
          }
      ...
      function selectBreakpointIVs(e) {
        ...
        if (pokemon) {
          ...
          $(".poke.single")
            .first()
            .find("input.iv[iv='atk']")
            .val(pokemon.ivs.atk);
          $(".poke.single")
            .first()
            .find("input.iv[iv='def']")
            .val(pokemon.ivs.def);
          $(".poke.single")
            .first()
            .find("input.iv[iv='hp']")
      ...
      function timelineEventClick(e) {
        ...
        if ($(this).hasClass("shield")) {
          ...
          $(
            ".timeline .charged[turn='" + turn + "'][actor='" + actor + "']"
          ).trigger("click");
          return;
        }
        ...
        for (var i = 0; i < pokemon.chargedMoves.length; i++) {
          $(".modal .move-select").append(
            '<option class="' +
              pokemon.chargedMoves[i].type +
              '" name="' +
              pokemon.chargedMoves[i].name +
              '" value="' +
              pokemon.chargedMoves[i].moveId +
              '">' +
              pokemon.chargedMoves[i].name +
              "</option>"
          );
          ...
          if (
            parseInt($(this).attr("energy")) < pokemon.chargedMoves[i].energy
          ) {
            $(".modal .move-select option")
              .last()
        ...
        $('.modal .move-select option[name="' + moveName + '"]').prop(
          "selected",
          "selected"
        );
        ...
        if ($(this).hasClass("charged") || $(this).hasClass("wait")) {
          for (var i = 0; i < actions.length; i++) {
            if (
              ...
              actions[i].turn == parseInt($(this).attr("turn"))
            ) {
          ...
          if (sandboxAction.settings.charge) {
            var chargeInt = chargeMultipliers.indexOf(
              sandboxAction.settings.charge
            );
            $('.modal .charge-select option[value="' + chargeInt + '"]').prop(
              "selected",
              "selected"
            );
          }
      ...
      function selectSandboxMove(e) {
        ...
        if (moveId != "wait") {
          ...
          if (move.energyGain > 0) {
            ...
            $(".modal .stat-dpe span").html(
              Math.round((move.damage / move.energy) * 100) / 100
            );
          }
      ...
      function selectSandboxChargePower(e) {
        ...
        var moveId = $(".modal .move-select option:selected").val();
        var chargeIndex = parseInt(
          $(".modal .charge-select option:selected").val()
        );
        ...
        $(".modal .stat-dpe span").html(
          Math.round((damage / move.energy) * 100) / 100
        );
