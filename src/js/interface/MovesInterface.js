    ...
    function interfaceObject() {
      ...
      this.displayMoves = function() {
        ...
        for (var i = 0; i < gm.data.moves.length; i++) {
          ...
          if (mode == "fast") {
            ...
            obj.dpt =
              Math.floor((move.power / (move.cooldown / 500)) * 100) / 100;
            obj.ept =
              Math.floor((move.energyGain / (move.cooldown / 500)) * 100) / 100;
      ...
      this.generateExploreResults = function(moveChange = false) {
        ...
        if (selectedCount == 2) {
          ...
          var chargedMultipliers =
            parseFloat(
              $(".effectiveness-select.charged option:selected").val()
            ) * ($(".stab.check.charged").hasClass("on") ? 1.2 : 1);

          var fastMovesPerCycle = Math.ceil(
            chargedMove.energy / fastMove.energyGain
          );
          var cycleDuration = fastMovesPerCycle * fastMove.cooldown + 500;
          var cycleDurationStr =
            cycleDuration / 500 + " turns (" + cycleDuration / 1000 + " s)";
          var fastDamage =
            Math.round(
              fastMovesPerCycle * fastMove.power * fastMultipliers * 10
            ) / 10;
          var chargedDamage =
            Math.round(chargedMove.power * chargedMultipliers * 10) / 10;
          var cycleDamage = Math.round((fastDamage + chargedDamage) * 10) / 10;
          var cycleDPT =
            Math.round((cycleDamage / (cycleDuration / 500)) * 100) / 100;
        ...
        for (var i = 0; i < movesetStats.length; i++) {
          var $stat = $(
            '<div class="stat"><h3>' +
              movesetStats[i].title +
              "</h3>" +
              "<span>" +
              movesetStats[i].value +
              "</span></div>"
          );
        ...
        if (moveChange) {
          ...
          for (var i = 0; i < gm.data.pokemon.length; i++) {
            ...
            if (
              fastMoveId != "" &&
              pokemon.fastMoves.indexOf(fastMoveId) == -1
            ) {
              valid = false;
            }
            ...
            if (
              chargedMoveId != "" &&
              pokemon.chargedMoves.indexOf(chargedMoveId) == -1
            ) {
              valid = false;
            }
            ...
            if (valid) {
              var rankLink =
                host + "rankings/all/1500/overall/" + pokemon.speciesId;

              var $rank = $(
                '<a href="' +
                  rankLink +
                  '" target="_blank" class="rank ' +
                  pokemon.types[0] +
      ...
      function getStatusEffectString(move) {
        ...
        for (var i = 0; i < stringArray.length; i++) {
          stringArray[i] =
            '<div class="status-effect-description">' +
            stringArray[i] +
            "</div>";
        }
