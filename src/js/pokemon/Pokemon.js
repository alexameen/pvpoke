...
function Pokemon(id, i, b) {
  ...
  self.fastMovePool.sort((a, b) =>
    a.moveId > b.moveId ? 1 : b.moveId > a.moveId ? -1 : 0
  );
  self.chargedMovePool.sort((a, b) =>
    a.moveId > b.moveId ? 1 : b.moveId > a.moveId ? -1 : 0
  );
  ...
  this.initialize = function(targetCP, defaultMode) {
    defaultMode =
      typeof defaultMode !== "undefined" ? defaultMode : "gamemaster";
      ...
      switch (defaultMode) {
        case "scale":
          ...
          while (iv < 15 && targetCPM > 0.7903) {
            ...
            targetCPM = Math.sqrt(
              (targetCP * 10) /
                ((this.baseStats.atk + iv) *
                  Math.pow(this.baseStats.def + iv, 0.5) *
                  Math.pow(this.baseStats.hp + iv, 0.5))
            );
          }
    ...
    this.stats.hp = Math.max(
      Math.floor(this.cpm * (this.baseStats.hp + this.ivs.hp)),
      10
    );
  ...
  this.calculateCP = function(
    ...
    var cp = Math.floor(
      ((self.baseStats.atk + atk) *
        Math.pow(self.baseStats.def + def, 0.5) *
        Math.pow(self.baseStats.hp + hp, 0.5) *
        Math.pow(cpm, 2)) /
        10
    );
  ...
  this.generateIVCombinations = function(
    sortStat,
    sortDirection,
    resultCount,
    filters
  ...
  this.calculateBreakpoints = function(defender) {
    ...
    var minDamage = battle.calculateDamageByStats(
      minAttack,
      defender.stats.def,
      effectiveness,
      self.fastMove
    );
    var maxDamage = battle.calculateDamageByStats(
      maxAttack,
      defender.stats.def,
      effectiveness,
      self.fastMove
    ...
    for (var i = minDamage; i <= maxDamage; i++) {
      var breakpoint = battle.calculateBreakpoint(
        i,
        defender.stats.def,
        effectiveness,
        self.fastMove
      );
      var maxDefenseBreakpoint = battle.calculateBreakpoint(
        i,
        maxDefense,
        effectiveness,
        self.fastMove
  ...
  this.calculateBulkpoints = function(attacker) {
    ...
    for (var i = minDamage; i <= maxDamage; i++) {
      var bulkpoint = battle.calculateBulkpoint(
        i,
        attacker.stats.atk,
        effectiveness,
        attacker.fastMove
      );
      var maxAttackBulkpoint = battle.calculateBulkpoint(
        i,
        maxAttack,
        effectiveness,
        attacker.fastMove
  ...
  this.resetMoves = function() {
    ...
    if (this.chargedMoves.length > 0) {
      ...
      self.activeChargedMoves.sort((a, b) =>
      ...
      if (self.activeChargedMoves.length > 1) {
        if (
          self.activeChargedMoves[1].energy ==
            self.activeChargedMoves[0].energy &&
          self.activeChargedMoves[1].buffs
        ) {
      ...
      for (var i = 0; i < self.activeChargedMoves.length; i++) {
        ...
        if (move.dpe - self.bestChargedMove.dpe > 0.03) {
  ...
  this.initializeMove = function(move) {
    ...
    if (move.energy > 0) {
      ...
      if (move.buffs) {
        ...
        var multiplier =
          (gm.data.settings.buffDivisor + buffEffect * move.buffApplyChance) /
          gm.data.settings.buffDivisor;
  ...
  this.selectRecommendedMoveset = function() {
    ...
    for (var i = 0; i < rankings.length; i++) {
      ...
      if (r.speciesId == self.speciesId) {
        ...
        self.selectMove(
          "charged",
          self.chargedMovePool[moveIndexes[1] - 1].moveId,
          0
        );
        ...
        if (moveIndexes[2] != 0) {
          self.selectMove(
            "charged",
            self.chargedMovePool[moveIndexes[2] - 1].moveId,
            1
          );
        }
  ...
  this.hasBuffMove = function() {
    ...
    for (var i = 0; i < self.chargedMoves.length; i++) {
      if (
        self.chargedMoves[i].buffs &&
        self.chargedMoves[i].buffApplyChance < 1
      ) {
        hasBuffMove = true;
      }
    }
  ...
  this.setLevel = function(amount, initialize) {
    ...
    if (index % 1 == 0) {
      ...
      // Set CPM for half levels
      this.cpm = Math.sqrt(
        (Math.pow(cpms[Math.floor(index)], 2) +
          Math.pow(cpms[Math.ceil(index)], 2)) /
          2
      );
    }
  ...
  this.generateURLPokeStr = function(context) {
    ...
    if (
      ...
      arr.push(
        ...
        self.ivs.def,
        self.ivs.hp,
        self.startStatBuffs[0] + gm.data.settings.maxBuffStages,
        self.startStatBuffs[1] + gm.data.settings.maxBuffStages,
        self.baitShields ? 1 : 0
      );
  ...
  this.generateURLMoveStr = function() {
    ...
    var chargedMove1Index =
      self.chargedMovePool.indexOf(self.chargedMoves[0]) + 1;
    var chargedMove2Index =
      self.chargedMovePool.indexOf(self.chargedMoves[1]) + 1;
